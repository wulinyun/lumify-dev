...
if [ ! -f "$ARCHIVE_DIR/accumulo-1.6.1-bin.tar.gz" ]; then
    curl -L -o $ARCHIVE_DIR/accumulo-1.6.4-bin.tar.gz http://apps.k8stest.landaudev.com/lumify/accumulo-1.6.4-bin.tar.gz
fi

# extract from the archive
tar -xzf $ARCHIVE_DIR/accumulo-1.6.4-bin.tar.gz -C /opt

# delete the archive
rm -rf $ARCHIVE_DIR

# build the package
ln -s /opt/accumulo-1.6.4 /opt/accumulo
rm -rf /opt/accumulo-1.6.4/logs
mkdir -p /var/log/accumulo
ln -s /var/log/accumulo /opt/accumulo-1.6.4/logs
